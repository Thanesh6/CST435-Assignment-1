...
def feature_service(data):
    ...
    features = np.concatenate([eigvals.real[:10], [rank]])
    print(f"[RANK 3] Extracted features (rank={rank})")
    return features
...
if rank == 0:
    ...
    # Send command to DataGen (Rank 1)
    comm.send(data_size, dest=1, tag=11)
    
    # Receive final results from Analysis (Rank 4)
