...
class MasterServicer(pipeline_pb2_grpc.MasterServiceServicer):
    def RunPipeline(self, request, context):
        ...
        # Connect to each service using their service names from docker-compose
        data_stub = pipeline_pb2_grpc.DataGenServiceStub(grpc.insecure_channel('datagen:50051'))
        pre_stub = pipeline_pb2_grpc.PreprocessServiceStub(grpc.insecure_channel('preprocess:50052'))
        feat_stub = pipeline_pb2_grpc.FeatureServiceStub(grpc.insecure_channel('feature:50053'))
        ana_stub = pipeline_pb2_grpc.AnalysisServiceStub(grpc.insecure_channel('analysis:50054'))
